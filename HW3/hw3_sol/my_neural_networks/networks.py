...
class BasicNeuralNetwork:
    ...
    def _new_mask(self, i, shape):
        if self.gpu_id != -1:
            m = torch.from_numpy(np.random.binomial(1, 1 - self.dropout_rate,
                                                size=shape)).float().cuda(self.gpu_id)
        else:
            m = torch.from_numpy(np.random.binomial(1, 1 - self.dropout_rate,
                                                size=shape)).float()
        return m
    ...
    def _feed_forward(self, X, skip_dropout=False):
        ...
        for i, (w, b) in enumerate(zip(self.weights, self.biases)):
            ...
            if i == len(self.weights) - 1:
                ...
                # dropout
                if not skip_dropout and self.dropout_rate > 0:
                    # this might slow down the training a little bit.
                    mask = self._new_mask(i, act_output.shape)
                    act_output = act_output * mask / (1 - self.dropout_rate)
                act_outputs.append(act_output)
    ...
    def _backpropagation(self, outputs, act_outputs, X, y_1hot):
        ...
        for i in range(len(self.weights)-1, 0, -1):
            ...
            if self.l2_lambda != 0:
                W_grad += self.l2_lambda * self.weights[i]
        ...
        if self.l2_lambda != 0:
            W_grad += self.l2_lambda * self.weights[0]
        b_grad = torch.sum(delta, 1) / m
    ...
    def train_one_batch(self, X, y, y_1hot):
        ...
        if self.l2_lambda != 0:
            loss += self.l2_lambda * self._l2_loss()
